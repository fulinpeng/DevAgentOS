...
2. **测试**：编写或修改测试时，必须先保证测试 **表达的是需求**；若发现测试与需求矛盾，**改测试**。**前端默认只做可被 Vitest 独立执行的 JS/TS 逻辑单测**（纯函数、\`renderHook\`、工具模块、storage 与数据流封装等），**不要**用整块页面的 DOM/UI 或「点按钮式」交互去反推需求。
3. **实现**：在测试正确反映需求的前提下，用业务代码满足测试；失败时先判断「测错了」还是「逻辑未实现」，再动对应一侧。
4. **测试风格（重要）**：**不要**新增或依赖 **交互式 UI 测试**（如 \`userEvent\`、\`fireEvent\`、整页 \`render\` + \`screen.getBy*\` / getByRole 驱动的点选流程）；**不要**把验收绑在 Testing Library 查 DOM 上，除非任务描述**明文要求**界面/无障碍回归。逻辑放在可测模块里，用 **纯逻辑单测 + \`renderHook\`** 等覆盖即可。
...
export function formatMinimalWorkerScopeBlock(): string {
  return [
    ...
    '- **writeFile** 优先最小 diff；**禁止**无任务依据时整份替换测试套件或堆交互式 UI 用例。**修复测试失败时优先**把逻辑抽到 hook/工具函数并写 **JS 逻辑单测**，**删除或改写**已有的 \`userEvent\`/整页 RTL 类测试；除非任务描述**明文要求**保留界面级回归，否则不要补 DOM 查询式用例。',
