...
2. **测试**：编写或修改测试时，必须先保证测试 **表达的是需求**；若发现测试与需求矛盾，**改测试**，不要用当前 UI 反推测试。
3. **实现**：在测试正确反映需求的前提下，用 **UI/业务代码** 满足测试；测试失败时优先判断属于「测错了」还是「没实现需求」，再动对应一侧。
4. **UI 检测保留**：实现与排错时必须对照组件/页面的 **真实界面契约**（可见文案、role、label、placeholder、布局与交互路径）；写 RTL 查询、改断言或改组件前，以源码与 DOM 行为为据，避免脱离界面的空想。
5. 禁止为凑绿而让测试盲从现有界面，或在不核对需求时削弱断言。
